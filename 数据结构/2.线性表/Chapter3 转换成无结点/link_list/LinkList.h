...
// 操作结果：根据数组v中的元素构造单链表
{
	...
	head=p=new Node<ElemType>(v[0],NULL);
	for (int i = 1; i < n; i++) {
...
//	i合法时函数返回SUCCESS,否则函数返回RANGE_ERROR
{
	if (i < 1 || i > length)
		...
		int count;
		if(i==1){head=head->next;delete p;}
		else
        {
            for (count = 1; count < i-1; count++) p = p->next;	      // p指向第i-1个结点
            q = p->next;	      // q指向第i个结点
            p->next = q->next;	  // 删除结点
...
//	i合法时返回SUCCESS, 否则函数返回RANGE_ERROR
{
	if (i < 1 || i > length+1)
		...
		Node<ElemType> *p = head, *q;
		int count;
		if(i=1){q = new Node<ElemType>(e,p->next);head=q;}
		else
        {
